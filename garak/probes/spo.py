...
class _SPOIntentAugmentedBase(SPOIntent):
    ...
    def _get_prompts(
        self, apply_to_system, apply_to_user, stub: str
            ...
            metadata.append(
                {
                    ...
                    "augmentation_target": (
                        "both"
                        if apply_to_system and apply_to_user
                        else "system" if apply_to_system else "user"
                    ),
                }
            )
    ...
    def _prompts_from_stub(self, stub: Stub) -> List[str]:
        ...
        prompts, metadata = self._get_prompts(
            self.augment_system, self.augment_user, stub.content
        )
    ...
    def _attempt_prestore_hook(
        ...
        if seq in self.prompt_to_variant:
            ...
            if "augmentation" in variant_info:
                ...
                attempt.notes["augmentation_target"] = variant_info[
                    "augmentation_target"
                ]
